...
def get_model(name, **kwargs):
    ...
    if name == "nn":
        ...
        optimizer = optim.SGD(model.parameters(), lr=lr)
        center_pixel = False
        # criterion = nn.BCELoss(weight=kwargs["weights"]) #二进制交叉熵
        criterion = nn.CrossEntropyLoss(weight=kwargs["weights"])
    else:
...
class LuoEtAl(nn.Module):
    ...
    # @staticmethod：在不实例化类的情况下直接访问该方法
    @staticmethod
    def weight_init(m):
...
class RCNN_UNetEtAl(nn.Module):
    ...
    def __init__(self, input_channels, n_classes, patch_size=32):
        ...
        self.upsamping1 = nn.ConvTranspose2d(512, 256, (2,2), 2)
        ...
        self.conv5 = nn.Conv2d(128, 256, (3, 3), 1, 1)
        self.conv6 = nn.Conv2d(256, 256, (3, 3), 1, 1)
        self.conv7 = nn.Conv2d(256, 512, (3, 3), 1, 1)
        self.conv8 = nn.Conv2d(512, 512, (3, 3), 1, 1)
        self.conv9 = nn.Conv2d(64, n_classes, (3, 3), 1, 1)
        self.conv10 = nn.Conv2d(512, 256, (3,3), 1, 1)
        self.conv11 = nn.Conv2d(256, 128, (3,3), 1, 1)
    ...
    def forward(self, x):
        ...
        x = self.BN1(x)
        x = self.ReLU(x)
        y1 = self.conv2(x) + x1  # concatenate1
        x = self.pool(y1)
        #encoder_2
        x2 = self.conv3(x)  # 128
        x = self.BN2(x2)
        ...
        x = self.BN2(x)
        x = self.ReLU(x)
        y2 = self.conv4(x) + x2  # concatenate2
        x = self.pool(y2)
        #encoder_3
        x3 = self.conv5(x)  # 256
        x = self.BN3(x3)
        ...
        x = self.BN3(x)
        x = self.ReLU(x)
        y3 = self.conv6(x) + x3  # concatenate3
        x = self.pool(y3)
        #bridge
        x4 = self.conv7(x) # 512
        x = self.BN4(x4)
...
def train(
    ...
    display=None,
    val_loader=None,  # 验证集
    supervision="full",  # full/semi 监督/半监督
):
    """
    ...
    for e in tqdm(range(1, epoch + 1), desc="Training the network"):
        ...
        for batch_idx, (data, target) in tqdm(
            ...
            avg_loss += loss.item()  # .item()减少显存占用
...
def test(net, img, hyperparams):
    ...
    """
    net.eval()  # 关闭dropout,当模型中存在dropout和batchnorm,测试时加上net.eval
